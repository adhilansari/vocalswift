...
export class JobController {
  ...
  async isolateAudio(
    ...
    let targetFilePath = join(
      process.cwd(),
      'uploads',
      isYoutube ? `${fileId}.mp3` : fileId,
    );
    ...
    if (isYoutube && !existsSync(targetFilePath)) {
      ...
      if (!res.ok)
        return {
          error: 'Failed to fetch youtube preview from separation service',
        };
    ...
    if (start !== undefined && end !== undefined) {
      ...
      try {
        await execAsync(
          `ffmpeg -y -i "${targetFilePath}" -ss ${start} -to ${end} "${trimmedPath}"`,
        );
  ...
  getEvents(@Param('id') id: string): any {
    ...
    return interval(1000).pipe(
      ...
      filter((status: any) => {
        ...
        const progNum = progObj
          ? typeof progObj === 'number'
            ? progObj
            : progObj.percent
          : 0;
  ...
  async downloadResult(@Param('id') id: string, @Res() res: Response) {
    ...
    if (existsSync(mp3Path)) {
      ...
      return res
        ...
        .json({ error: 'File not found or not ready' });
    }

    const job = await this.jobService.getRawJob(id);
    let originalName = 'vocals';
    if (job && job.data) {
      originalName = job.data.isYoutube
        ? 'youtube_audio'
        : job.data.originalName || 'vocals';
    }
  ...
  async trimResult(
    ...
    try {
      await execAsync(
        `ffmpeg -y -i "${filePath}" -ss ${start} -to ${end} "${trimmedPath}"`,
      );
  ...
  async trimSilenceResult(@Param('id') id: string) {
    ...
    try {
      await execAsync(
        `ffmpeg -y -i "${filePath}" -af "silenceremove=start_periods=1:start_duration=0.5:start_threshold=-50dB,areverse,silenceremove=start_periods=1:start_duration=0.5:start_threshold=-50dB,areverse" "${trimmedPath}"`,
      );
      return { resultUrl: `/api/jobs/download/${trimmedId}` };
    } catch (e) {
      console.error(e);
  ...
  async cutResult(
    ...
    try {
      if (start <= 0.1) {
        ...
        await execAsync(
          `ffmpeg -y -i "${filePath}" -filter_complex "${filter}" -map "[out]" "${cutPath}"`,
        );
      }
